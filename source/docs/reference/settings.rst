...
``ROOT_API_PACKAGE``
   *Package name. Default: unset.* When this is set, we do *not* generate base API
   ...
   to work, such as ``Node getParent()`` the node subclasses have to have the common
   ...
   (*NB: This whole issue of multiple grammar/parsers co-existing really needs its own
   section, I think. (JR)*)
